...
HTMLElement.prototype.simpleSyntax = function(options) {
	...
	if (options.width === undefined)
        ...
        codeDiv.style.width = 'calc(100% - 12px)';    
    ...
    if (options.copyButton) {
        ...
        copyButton.addEventListener('click', function() {
            let prevSelStart = textarea.selectionStart;
            let prevSelEnd = textarea.selectionEnd;
            textarea.select();
            document.execCommand('copy');
            textarea.selectionStart = prevSelStart;
            textarea.selectionEnd = prevSelEnd;
        });
	}
	...
	if (options.width === undefined)
        ...
        searchbarDiv.style.width = options.width;
	...
	function highlightSyntax() {
		...
		if (options.language != "none") {
			...
			for (let i = 0; i < code.length; i++) {
				...
				if (i > 0 && code[i] !== '\\') {
                    if ((openedSingleQuote || openedDoubleQuote || isMultilineCommentOpening) && code[i] == '/' && code[i-1] == '/')
                    {
		...
		if (options.language == "html" || options.language == "xml" || options.language == "php") {
        ...
        } if (options.language == "java" || options.language == "javascript" || options.language == "csharp" || options.language == "python" || options.language == "kotlin" || options.language == "cpp") {
            ...
            if (options.language != "python")
                text = text.replace(/\/\*[\s\S]*?\*\//g, '<span class="ss-comment">$&</span>');
			text = text.replace(/("[^"]*"|'[^']*')|(\{|\})/g, function(match, quoted, group) {
			...
			var varTypes = /\b(int|float|bool|object|callable|iterable|null)\b/g;
			var keywords = /\b(abstract|and|array|as|break|case|catch|clone|continue|declare|default|do|echo|else|elseif|final|finally|for|foreach|function|global|goto|if|implements|include|include_once|interface|isset|list|new|or|print|private|protected|public|require|require_once|return|static|switch|throw|try|unset|use|while|xor|yield|true|false|null)\b/g;


			text = text
				.replace(keywords, '<span class="ss-php-keyword">$1</span>')
				.replace(varTypes, '<span class="ss-php-type">$1</span>');

            text = text.replace(/\/\*[\s\S]*?\*\//g, '<span class="ss-php-comment">$&</span>');
			text = text.replace(/("[^"]*"|'[^']*')|(\{|\})/g, function(match, quoted, group) {
					if (quoted) return quoted;
					if (group) return '<span class="ss-php-brace ss-curly-brace">' + group + '</span>';
				})
				.replace(/("[^"]*"|'[^']*')|(\(|\))/g, function(match, quoted, group) {
					if (quoted) return quoted;
					if (group) return '<span class="ss-php-brace ss-round-brace">' + group + '</span>';
				});

            text = text.replace(/(\/\/.*?)(?=\r?\n|<br>|$)/g, '<span class="ss-php-comment">$1</span>');

			text = text
                .replace(/(\$[a-zA-Z_\x80-\xff][a-zA-Z0-9_\x80-\xff]*)/g, '<span class="ss-php-type">$1</span>');
